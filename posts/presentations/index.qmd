...
targets::tar_load(indices_of_interest, store = projects$preproc_behav$store)
targets::tar_load(users, store = projects$preproc_behav$store)
...
::: {layout-ncol=2}
![Average across 4-10 factors](../../.output/factcons_exclude-11.png){width=40%}

![Average across 8-10 factors](../../.output/factcons_nfact-large_exclude-11.png){width=40%}
:::
...
dims <- read_csv("config/dimensions.csv", show_col_types = FALSE)
dims |> 
...
params |> 
  ...
  as_tibble() |> 
  left_join(dims, by = join_by(From == game_index)) |> 
  mutate(
    game_name = str_remove(game_name, "[a-zA-Z]+$"),
    dim_name = factor(dim_name, unique(dims$dim_name)),
    dim_name_disp = fct_reorder(
      str_glue("{dim_name} ({dim_abbr})"),
      as.integer(dim_name)
    )
  ) |> 
...
cormat_latents |> 
  ...
  as.dist() |> 
  cmdscale() |> 
  as_tibble(rownames = "dim_abbr") |> 
  ggplot(aes(V1, V2)) +
  geom_point() +
  ggrepel::geom_text_repel(aes(label = dim_abbr)) +
  theme_void()
```
...
![](../../.output/corr_slices_g_dur_type-total.png)

## Example: $R^2$ with G

![](../../.output/corr_slices_g_dur_type-per_min.png)
